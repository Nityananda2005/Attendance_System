...
const FacultyProfile = () => {
  ...
  const menuItems = [
    { icon: Settings, label: 'Profile Settings', desc: 'Update your personal and professional details' },
    { icon: Bell, label: 'Notification Preferences', desc: 'Manage alerts for sessions and attendance' },
    { icon: Lock, label: 'Security & Privacy', desc: 'Change password and manage account security' },
  ];
      ...
      <div className="max-w-4xl mx-auto space-y-8 pb-20 pt-6 px-4">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
            <div className="relative">
              <div className="w-32 h-32 rounded-[2rem] overflow-hidden border-4 border-white dark:border-slate-800 shadow-xl">
                 <img
                   src={`https://ui-avatars.com/api/?name=${encodeURIComponent(displayUser?.name || 'Faculty')}&background=3b82f6&color=fff&size=200`}
                   alt="Faculty Avatar"
            ...
            <div className="text-center md:text-left space-y-2">
              <h1 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{displayUser?.name || 'Faculty Member'}</h1>
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                ...
                <span className="flex flex-wrap items-center gap-1.5 text-sm font-medium">
                  ...
                  {Array.isArray(displayUser?.department) && displayUser.department.length > 0 ? (
                    displayUser.department.map((dept, idx) => (
                      <span key={idx} className="px-2 py-0.5 bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 text-[10px] font-bold rounded-lg whitespace-nowrap">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10 pt-8 border-t border-gray-100 dark:border-slate-800/50">
             <div className="flex items-center gap-4 p-4 bg-gray-50/50 dark:bg-slate-800/30 rounded-2xl border border-gray-100/50 dark:border-slate-700/30">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm font-bold dark:text-white truncate">{displayUser?.email || 'faculty@attendtrack.com'}</p>
                </div>
             </div>
             <div className="flex items-center gap-4 p-4 bg-gray-50/50 dark:bg-slate-800/30 rounded-2xl border border-gray-100/50 dark:border-slate-700/30">
                <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm text-gray-400">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold dark:text-white">{joiningDate}</p>
                </div>
             </div>
             <div className="flex items-center gap-4 p-4 bg-gray-50/50 dark:bg-slate-800/30 rounded-2xl border border-gray-100/50 dark:border-slate-700/30">
                <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm text-gray-400">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Assigned Departments</p>
                  <div className="flex flex-wrap gap-2">
                    {Array.isArray(displayUser?.department) && displayUser.department.length > 0 ? (
                      displayUser.department.map((dept, idx) => (
                        <span key={idx} className="px-3 py-1 bg-blue-500/10 text-blue-600 dark:text-blue-400 text-xs font-bold rounded-xl border border-blue-500/20">
                          {dept}
                        </span>
                      ))
                    ) : (
                      <p className="text-sm font-bold dark:text-white italic">Not Assigned</p>
                    )}
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
           ...
           <div className="space-y-4">
              <h3 className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] px-2">Professional Info</h3>
              <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/20 dark:border-slate-800/20 rounded-[2rem] p-6 shadow-xl space-y-6">
                 <div className="space-y-4">
                    ...
                    {[
                      { l: 'Sessions Today', v: '4', c: 'text-blue-500' },
                      { l: 'Avg. Attendance', v: '88%', c: 'text-emerald-500' },
                      { l: 'Pending Reviews', v: '2', c: 'text-amber-500' }
                    ].map((m, i) => (
